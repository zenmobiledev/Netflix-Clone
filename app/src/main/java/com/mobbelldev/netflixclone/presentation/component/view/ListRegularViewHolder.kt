package com.mobbelldev.netflixclone.presentation.component.view

import android.view.View
import androidx.recyclerview.widget.RecyclerView
import com.mobbelldev.netflixclone.R
import com.mobbelldev.netflixclone.presentation.widget.regular.MovieListRegularWidget

class ListRegularViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
    fun bind(category: String) {
        val regularListWidget =
            itemView.findViewById<MovieListRegularWidget>(R.id.widget_list_regular)
        regularListWidget.setCategory(category)
    }
}